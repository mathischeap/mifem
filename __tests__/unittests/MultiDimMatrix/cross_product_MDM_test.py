...
from objects.CSCG._3d.__tests__.Random.field import random_vector as rv3
...
def test_MDM_sf_CrossProduct():
    ...
    w.TW.func.do.set_func_body_as(s2)
    w.TW.current_time = 0
    w.TW.do.push_all_to_instant()
    w.discretize()

    u.TW.func.do.set_func_body_as(v2)
    u.TW.current_time = 0
    u.TW.do.push_all_to_instant()
    u.discretize()

    v.TW.func.do.set_func_body_as(V2)
    v.TW.current_time = 0
    v.TW.do.push_all_to_instant()
    v.discretize()
    ...
    w.TW.func.do.set_func_body_as(s3)
    w.TW.current_time = 0
    w.TW.do.push_all_to_instant()
    w.discretize()

    u.TW.func.do.set_func_body_as(v3)
    u.TW.current_time = 0
    u.TW.do.push_all_to_instant()
    u.discretize()

    v.TW.func.do.set_func_body_as(V3)
    v.TW.current_time = 0
    v.TW.do.push_all_to_instant()
    v.discretize()
