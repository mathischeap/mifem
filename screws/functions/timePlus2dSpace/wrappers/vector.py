...
from screws.functions.timePlus2dSpace.wrappers.helpers.scalar_add import t2d_ScalarAdd
from screws.functions.timePlus2dSpace.wrappers.helpers.scalar_sub import t2d_ScalarSub
from screws.functions.timePlus2dSpace.wrappers.helpers.scalar_neg import t2d_ScalarNeg
from screws.functions.timePlus2dSpace.wrappers.helpers.scalar_mul import t2d_ScalarMultiply

from screws.numerical.timePlus2dSpace.partial_derivative_as_functions import NumericalPartialDerivative_txy_Functions
...
class t2dVector(FrozenOnly):
    ...
    def visualize(self, mesh, t):
        ...
        if mesh.__class__.__name__ == "_2dCSCG_Mesh":
            scalar = _2dCSCG_VectorField(mesh, [self._v0_, self._v1_])
            scalar.current_time = t
            return scalar.visualize

        elif mesh.__class__.__name__ == "miUsGrid_TriangularMesh":
            scalar = miUsGrid_Triangular_Vector(mesh, [self._v0_, self._v1_])
            scalar.current_time = t
