...
class InnerMassMatrixGenerator(FrozenOnly):
    ...
    def __call__(self, rp):
        ...
        M00 = self.___Pr_mh1___(weights, sqrtg*g[0][0], basis[0], basis[0])
        M11 = self.___Pr_mh1___(weights, sqrtg*g[1][1], basis[1], basis[1])
        mark = self._mesh_[rp].type_wrt_metric.mark
        if isinstance(mark, str) and mark[:4] == 'Orth':
            M01 = None
            M10 = None
        else:
            M01 = self.___Pr_mh1___(weights, sqrtg*g[0][1], basis[0], basis[1])
            M10 = M01.T
        Mi = bmat([(M00, M01),
                   (M10, M11)], format='csr')
        return Mi
