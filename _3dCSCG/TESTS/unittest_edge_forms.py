...
def test_edge_forms_No0_save_read():
    ...
    E0, E00 = read('_3dCSCG/TESTS/__unittest_files__/___unittest_edge_save_read_000___.mi')
    if 100 in E0.cochain.local:
        np.testing.assert_array_almost_equal(E0.cochain.local[100],
            np.array([10.16985183,  0.97463385, -7.43387363, -0.12659664,  3.66254627,
                       ...
                       -7.43387363, -7.87427561, -0.12659664, -0.25492221,  7.12750344,
                        7.30898322]))
        np.testing.assert_array_almost_equal(E00.cochain.local[100],
            np.array([ 0.96194596,  0.69644976,  0.24434041,  0.03739119, -0.46726863,
    ...
    if 125 in E0.cochain.local:
        np.testing.assert_array_almost_equal(E0.cochain.local[125],
            np.array([ 0.06165154, -1.07090409, -0.57134205,  7.3344841 ,  9.12644186,
                   ...
                   13.50683212]))
        np.testing.assert_array_almost_equal(E00.cochain.local[125],
            np.array([-0.738119  , -0.97656508, -0.95334133, -0.86974113, -0.50646381,
    ...
    assert E00.TW.current_time == 2.875, f"we have restored the TW.func."
    assert E00.TW.func.body.__class__.__name__ == "_3dCSCG_ScalarField", f"we must have restored the func."
    ...
    assert E0.TW.current_time == t + 10
    assert E0.TW.func.body.__class__.__name__ == "_3dCSCG_ScalarField", f"we must have restored the func."
...
if __name__ == '__main__':
    # mpiexec -n 6 python _3dCSCG\TESTS\unittest_edge_forms.py
    test_edge_forms_No1_0edge_Rd_and_Rc()