# -*- coding: utf-8 -*-
"""
$ mpiexec python _3dCSCG\APP\contents\icpsNS\BC\pwc2.py
...
N, k, steps = int(N), int(k), int(steps)

conserving = False
RDF_filename = 'RDF_PWC2_N{}K{}Steps{}'.format(N, k, steps)
name         = 'Iterator_PWC2_N{}K{}Steps{}'.format(N, k, steps)
...
mesh = MeshGenerator('pwc', l=1, w=0.5, h=0.5)([2*k, k, [1,2,4,8,16,32,32,32,32,16,8,4,2,1]])
...
if start_with is None:
    u1.TW.func.DO_set_func_body_as(es.status.velocity)
    u2.TW.func.DO_set_func_body_as(es.status.velocity)
    w1.TW.func.DO_set_func_body_as(es.status.vorticity)
    w2.TW.func.DO_set_func_body_as(es.status.vorticity)
    u1.TW.current_time = t0
    w2.TW.current_time = t0
    w1.TW.current_time = t0
    u2.TW.current_time = t0
    u1.TW.DO_push_all_to_instant()
    w2.TW.DO_push_all_to_instant()
    w1.TW.DO_push_all_to_instant()
    u2.TW.DO_push_all_to_instant()
    u1.discretize()
    w2.discretize()
    w1.discretize()
    u2.discretize()
...
du2 = u2.coboundary()
du2.TW.func.DO_set_func_body_as(es.status.divergence_of_velocity)
du2.TW.current_time = t0
du2.TW.DO_push_all_to_instant()
...
del M2E21, N2_A, M1_A, lhs33_A, lhs00, E12M2E21, lhs00_Hf_A
...
def SOLVER(tk, tk1):
    ...
    if abs(round(tk1) - tk1) < 1e-8: save([u1, u2, w1, w2], f'PWC2_uuww_N{N}k{k}_t{int(round(tk1))}')
    ...
    if abs(round(tk1) - tk1) < 1e-8: save([u1, w2], f'PWC2_u1w2_N{N}k{k}_t{int(round(tk1))}h')
