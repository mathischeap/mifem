# -*- coding: utf-8 -*-
"""
$ mpiexec python _3dCSCG\APP\contents\icpsNS\BC\test_2\pwc2_test_2.py
...
# do not change below ....
N = 3
element_layout = [10, 6, [1,2,4,8,16,16,16,16,16,16,8,4,2,1]]
F = 1
nu = 0.1
rho = 1
restart = 300
...
conserving = False
RDF_filename = f'RDF_PWC2_TEST_2_t0{int(t0)}_t{int(t)}'
name         = f'Iterator_PWC2_TEST_2_t0{int(t0)}_t{int(t)}'
...
def SOLVER(tk, tk1):
    ...
    if abs(round(tk1) - tk1) < 1e-8: save([u1, u2, w1, w2],
        f'PWC2_uuww_TEST_2_t{int(round(tk1))}')
    ...
    if abs(round(tk1) - tk1) < 1e-8: save([u1, w2],
        f'PWC2_u1w2_TEST_2_t{int(round(tk1))}h')
