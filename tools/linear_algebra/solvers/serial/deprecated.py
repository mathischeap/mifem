...
def spsolve(AA, bb):
    ...
    assert bb.__class__.__name__ == 'GlobalVector' and bb.IS.master_dominating, \
...
def ___scipy_sparse_linalg___(solver_name, AA, bb, X0, restart=100, maxiter=1000, tol=1e-5):
    ...
    t0 = time()
    assert AA.__class__.__name__ == 'GlobalMatrix', \
        f" <serial> <scipy sparse linalg> <{solver_name}> A must be GlobalMatrix"
    assert bb.__class__.__name__ == 'GlobalVector', \
        f" <serial> <scipy sparse linalg> <{solver_name}> b must be GlobalVector"
    assert X0.__class__.__name__ == 'DistributedVector', \
        f" <serial> <scipy sparse linalg> <{solver_name}> x0 must be DistributedVector"
