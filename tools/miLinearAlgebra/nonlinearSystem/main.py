...
import numpy as np
from tools.miLinearAlgebra.nonlinearSystem.solve.main import NonLinearSystem_Solve
from tools.miLinearAlgebra.nonlinearSystem.nonliner_terms import nLS_NonlinearTerms
from tools.miLinearAlgebra.nonlinearSystem.num import nLS_num
from tools.miLinearAlgebra.nonlinearSystem.do import nLS_DO
from tools.miLinearAlgebra.nonlinearSystem.customize import nLS_Customize
    ...
    def ___PRIVATE_parse_nonlinear_terms___(self, terms):
        ...
        for i, row_terms in enumerate(terms):
            if row_terms == 0 or row_terms is None:
                pass
            elif row_terms.__class__.__name__ == 'MultiDimMatrix' and row_terms.whether.inhomogeneous:
                pass
            elif isinstance(row_terms, (list, tuple)):
                for term in row_terms:
                    if term.__class__.__name__ == 'MultiDimMatrix' and term.whether.inhomogeneous:
                        pass
                    else:
                        regularity = None
            else:
