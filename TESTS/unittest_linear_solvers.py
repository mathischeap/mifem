...
from TOOLS.linear_algebra.data_structures import GlobalVector, GlobalMatrix, LocallyFullVector
import TOOLS.linear_algebra.solvers.parallel.GMRES as GMRES
...
def test_LinearSolver_No0_GMRES():
    ...
    x0, info, beta, ITER, message = GMRES.solve(A, b, X0, restart=9, preconditioner=None, COD=False, routine='mpi_v2')
    x0 = x0.V
    ...
    x0, info, beta, ITER, message = GMRES.solve(A, b, X0, restart=9, preconditioner=None, COD=False, routine='mpi_v0')
    x0 = x0.V
    np.testing.assert_array_almost_equal(x0, np.array([-3.23891085,  3.44129703,  1.7765975 , -2.7063454 , -0.11510028,
                                                        0.94048189,  0.36495389,  0.54018445,  1.57663592]))

    x0, info, beta, ITER, message = ParallelSolverDistributor("GMRES")(A, b, X0, restart=9, preconditioner=('Jacobian', dict()),
                                                                       COD=False, routine='mpi_v0')
    x0 = x0.V
    np.testing.assert_array_almost_equal(x0, np.array([-3.23891085,  3.44129703,  1.7765975 , -2.7063454 , -0.11510028,
                                                        0.94048189,  0.36495389,  0.54018445,  1.57663592]))

    x0, info, beta, ITER, message = ParallelSolverDistributor("GMRES")(A, b, X0, restart=9, preconditioner=None, COD=False,
                                                                       routine='auto')
    x0 = x0.V
    np.testing.assert_array_almost_equal(x0, np.array([-3.23891085,  3.44129703,  1.7765975 , -2.7063454 , -0.11510028,
                                                        0.94048189,  0.36495389,  0.54018445,  1.57663592]))
    ...
    x0, info, beta, ITER, message = ParallelSolverDistributor("GMRES")(A, b, X0, restart=9, preconditioner=None, COD=False,
                                                                       routine='auto', loading_factor=0) # make sure we use parallel routine
    x0 = x0.V
    np.testing.assert_array_almost_equal(x0, np.array([-3.23891085,  3.44129703,  1.7765975 , -2.7063454 , -0.11510028,
                                                        0.94048189,  0.36495389,  0.54018445,  1.57663592]))
...
def test_LinearSolver_No1_BiCGSTAB():
    ...
    ParallelSolverDistributor("BiCGSTAB")(A, b, X0, maxiter=10, preconditioner=None, COD=False, routine='auto')
    x0 = x0.V
    np.testing.assert_array_almost_equal(x0, np.array([-2.1810344827586, 1.8362068965517, -0.5948275862068]))

    x0, info, beta, ITER, message = \
    ParallelSolverDistributor("BiCGSTAB")(A, b, X0, maxiter=10, preconditioner=('Jacobian', dict()), COD=False, routine='auto')
    x0 = x0.V
    np.testing.assert_array_almost_equal(x0, np.array([-2.1810344827586, 1.8362068965517, -0.5948275862068]))
...
def test_LinearSolver_No2_LooseGMRES():
    ...
    x0, info, beta, ITer, message = ParallelSolverDistributor("LGMRES")(A, b, X0, m=6, k=2, atol=1e-9,
                                                            maxiter=100, preconditioner=None, COD=False, routine='auto')
    x0 = x0.V
    np.testing.assert_array_almost_equal(x0, np.array([-3.23891085,  3.44129703,  1.7765975 , -2.7063454 , -0.11510028,
                                                        0.94048189,  0.36495389,  0.54018445,  1.57663592]))
