...
class CSCG_MESH_BASE(FrozenClass):
    ...
    def ___PRIVATE_BASE_get_region_elements_distribution_type___(self):
        ...
        the topology or whatever, we only care about the amount of elements in each regions:

            1). When the amount of elements in each regions is more or less the same,
                self._REDT_ = 'equal'
            else).
                self._REDT_ = 'unknown'
    ...
    def ___PRIVATE_BASE_decide_EDM___(self, EDM):
        ...
        if EDM in customized_methods: # we strongly use the method.
            ...
            if self._REDT_ == 'unknown':
                self._EDM_ = None # when Region_Elements_Distribution_Type is unknown, we use trivial elements division.
    ...
    def ___PRIVATE_BASE_analyze_element_distribution___(self):
        ...
        if not hasattr(self, '___USEFUL_regions_and_boundaries___'):
            ...
            assert self.___USEFUL_regions_and_boundaries___ == ___USEFUL_regions_and_boundaries___, "Can not change this property"



        if not hasattr(self, '_elements_in_regions_'):
            # this core's elements are in this regions (or these regions)
