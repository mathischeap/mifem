...
def Generating_Structured_Square_Mesh(K, d=2):
    ...
    if rAnk == mAster_rank:
        ...
        base_path = str(__file__).split('structured_square')[0]

        unstructuredGridToVTK(base_path + f'structured_square/K{K}d{d}',
                              x, y, z, CON, offsets, types)



if __name__ == "__main__":
    # mpiexec -n 4 python objects/miUsGrid/triangular/mesh/samples/structured_square.py
    Generating_Structured_Square_Mesh(8)
    Generating_Structured_Square_Mesh(16)
    Generating_Structured_Square_Mesh(64)
    Generating_Structured_Square_Mesh(128)