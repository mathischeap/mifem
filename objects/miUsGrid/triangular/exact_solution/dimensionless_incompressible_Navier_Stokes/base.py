...
import numpy as np
from objects.miUsGrid.triangular.exact_solution.base import miUsTriangle_ExactSolutionBase
...
class DimensionlessIncompressibleNavierStokes(miUsTriangle_ExactSolutionBase):
    """Dimensionless form."""
    ...
    def __init__(self, mesh, Re):
        ...
        Re
        """
        super(DimensionlessIncompressibleNavierStokes, self).__init__(mesh)
    ...
    @property
    def nu(self):
        """ 1 / Re, not the kinematic viscosity."""
        return self._nu_
    ...
    def divergence_of_velocity(self):
        if self._divergence_of_velocity_ is None:
            ...
            self._divergence_of_velocity_ = miUsGrid_Triangular_Scalar(self.mesh,
    ...
    def curl_of_omega(self):
        if self._curl_of_omega_ is None:
            self._curl_of_omega_ = miUsGrid_Triangular_Vector(
                self.mesh,
    ...
    def total_pressure(self):
        ...
        if self._total_pressure_ is None:
            self._total_pressure_ = miUsGrid_Triangular_Scalar(
    ...
    def kinetic_energy_distribution(self):
        ...
        if self._kinetic_energy_distribution_ is None:
            self._kinetic_energy_distribution_ = miUsGrid_Triangular_Scalar(
    ...
    def body_force(self):
        ...
        if self._body_force_ is None:
            self._body_force_ = miUsGrid_Triangular_Vector(
