...
class ___MassMatrix_Inner___(FrozenOnly):
    ...
    def ___Pr_operator_inner___(self, i, xietasigma, quad_weights, bf):
        ...
        M00 = self.___PRIVATE_inner_Helper1___(quad_weights, sqrt_g*g[0][0], bf[i][0], bf[i][0])
        M11 = self.___PRIVATE_inner_Helper1___(quad_weights, sqrt_g*g[1][1], bf[i][1], bf[i][1])
        M01 = self.___PRIVATE_inner_Helper1___(quad_weights, sqrt_g*g[0][1], bf[i][0], bf[i][1])
        M10 = M01.T
    ...
    def ___PRIVATE_inner_Helper1___(quad_weights, sqrt_g_g, bfO, bfS):
        M = np.einsum('m, im, jm -> ij', quad_weights * sqrt_g_g, bfO, bfS, optimize='greedy')
