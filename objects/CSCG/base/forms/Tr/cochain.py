...
class CSCG_TrForm_Cochain_BASE(FrozenOnly):
    """"""
    ...
    def EWC(self):
        ...
        ewc.gathering_matrix = self._Tr_
        return ewc
    ...
    def ___PRIVATE_do_gather_to_master_and_make_them_region_wise_local_index_grouped___(self):
        ...
        RN_LI_dict = dict()
        for i in self.local:
            rn, loc_ind = self._Tr_.mesh.do.find.region_name_and_local_indices_of_element(i)
        ...
        if rAnk == mAster_rank:
            ...
            for i in range(self._Tr_.mesh.elements.GLOBAL_num):
    ...
    def ___PRIVATE_do_distribute_region_wise_local_index_grouped_cochain_to_local___(self, RW_LI_COCHAIN):
        ...
        for i in self._Tr_.mesh.elements:
            rn, loc_ind = self._Tr_.mesh.do.find.region_name_and_local_indices_of_element(i)
    ...
    def globe(self):
        ...
        """
        GM = self._Tr_.numbering.gathering
        globe = lil_matrix((1, self._Tr_.GLOBAL_num_dofs))
        ...
        if rAnk == mAster_rank:
            measure = np.zeros(self._Tr_.GLOBAL_num_dofs, dtype=int)
            for G in GLOBE:
                indices = G.indices
                measure[indices] += 1
            ...
            globe = csc_matrix((self._Tr_.GLOBAL_num_dofs, 1))
    ...
    def globe(self, globe):
        if globe.__class__.__name__ == 'DistributedVector':
            assert globe.V.shape == (self._Tr_.GLOBAL_num_dofs, 1), "globe cochain shape wrong."
            ...
            if globe.IS_master_dominating:
                ...
                if rAnk == mAster_rank:
                    VV = np.empty((self._Tr_.GLOBAL_num_dofs,))
                    for v in V:
                        indices = v.indices
                        data = v.data
                        VV[indices] = data
            # distribute vector to individual cores ...
            local_range = self._Tr_.numbering.gathering.local_range
                ...
                for lr in local_range:
                    if lr == tuple():
                        ...
                        to_be_sent = spspa.csc_matrix(
                            ...
                            shape=(self._Tr_.GLOBAL_num_dofs, 1))
    ...
    def local(self, local):
        numOfElements = self._Tr_.mesh.elements.num
        numOfBasis = self._Tr_.num.basis
        try:
    ...
    def local_TEW(self, local_TEW):
        ...
        :return:
        """
        numOfBasis = self._Tr_.num.basis_onside
        try:
            assert isinstance(local_TEW, dict)
            for key in local_TEW:
                te = self._Tr_.mesh.trace.elements[key]
    ...
    def ___local_TEW_2_local___(self):
        """"""
        MAP = self._Tr_.mesh.trace.elements.map
        ...
        if self._Tr_.k == 2: # for 2-Tr-form, we can do below cause no dofs at the corner.......
            for i in MAP:
                ...
                assert local[i].shape == (self._Tr_.num.basis,)
        else:
