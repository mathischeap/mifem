...
@time: 12/8/2022 3:22 PM
"""
from components.freeze.main import FrozenOnly
import numpy as np


class _3dCSCG_Internal_Local_Dofs(FrozenOnly):
    """"""
    ...
    def _3dCSCG_1Form(self):
        ...
        return np.concatenate([ild0, ild1, ild2])

    @property
    def _3dCSCG_2Form(self):
        """"""
        local_numbering = self._space_.local_numbering._3dCSCG_2Form
        p0, p1, p2 = self._space_.p
        I_DOS = list()
        if p0 == 1:
            pass
        else:
            I_DOS.append(local_numbering[0][1:-1, :, :].ravel('F'))
        if p1 == 1:
            pass
        else:
            I_DOS.append(local_numbering[1][:, 1:-1, :].ravel('F'))
        if p2 == 1:
            pass
        else:
            I_DOS.append(local_numbering[2][:, :, 1:-1].ravel('F'))
        return np.concatenate(I_DOS)
