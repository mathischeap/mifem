...
class ___2dCSCG_2_i_Form_CrossProduct_2_X_1__ip_1___(FrozenOnly):
    """To compute (w2 X u1, e1). Cochain of w2 must be known, return mesh-element-wise
    matrices whose columns refer to local cochain of u1, rows' refer to local cochain
    of e1.
    """
    def __init__(self, w2, u1, e1, quad_degree=None):
        """
        ...
        :param w2: will be the 2-form itself. We call this function form it.
        :param u1:
        :param e1:
        :param quad_degree:
        """

        assert u1.ndim == w2.ndim == e1.ndim, " <___2dCSCG_2_i_Form_CrossProduct_2_X_1__ip_1___> "
        assert u1.k == e1.k == 1, " <___2dCSCG_2_i_Form_CrossProduct_2_X_1__ip_1___> "
        assert u1.mesh == w2.mesh, "___2dCSCG_2_i_Form_CrossProduct_2_X_1__ip_1___: Meshes do not match."
        assert u1.mesh == e1.mesh, "___2dCSCG_2_i_Form_CrossProduct_2_X_1__ip_1___: Meshes do not match."

        if quad_degree is None:
            quad_degree = [int(np.max([u1.dqp[i], w2.dqp[i], e1.dqp[i]])) * 2 for i in range(2)]
        ...
        xi = xi.ravel('F')
        et = et.ravel('F')
        detJ = w2.mesh.elements.coordinate_transformation.Jacobian(xi, et)
