...
from objects.CSCG._2d.mesh.periodic_setting.periodic_boundary_pairs.regular.main import _2dCSCG_Regular_PBP
...
class _2dCSCG_PeriodicDomainSetting(FrozenOnly):
    ...
    def ___INITIALIZING_INDIVIDUAL_BOUNDARY_PAIRS___(self):
        ...
        for pair in self._periodic_boundary_pairs_keys_:
            if self._periodic_boundary_pairs_keys_[pair] == 'regular':
                self._periodic_boundary_pairs_[pair] = _2dCSCG_Regular_PBP(self, pair)
    ...
    @property
    def periodic_region_edge_pairs(self):
        if self._periodic_region_edge_pairs_ is None:
            self._periodic_region_edge_pairs_ = dict()
            for pbp in self.periodic_boundary_pairs:
                self._periodic_region_edge_pairs_.update(
                    self.periodic_boundary_pairs[pbp].region_edge_pairs)
        return self._periodic_region_edge_pairs_