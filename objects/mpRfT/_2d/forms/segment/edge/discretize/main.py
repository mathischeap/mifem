...
from objects.mpRfT._2d.forms.segment.edge.discretize.scalar.standard import mpRfT2_ESgF_Discretize_Standard_Scalar



class mpRfT2_ESgF_Discretize(FrozenOnly):
    """"""

    def __init__(self, t):
        """"""
        self._t_ = t
        self._Pr_standard_scalar = mpRfT2_ESgF_Discretize_Standard_Scalar(t)
        self._freeze_self_()
        ...
        if target == 'func':
            ...
            if self._t_.TW.func.__class__.__name__ == 'mpRfT2_Scalar':
                ...
                if self._t_.TW.func.ftype == 'standard':
                    ...
                    raise NotImplementedError(f"mpRfT2_ESgF cannot (target func) "
                                              f"discretize mpRfT2_Scalar of ftype="
                                              f"{self._t_.TW.func.ftype}")

            else:
                raise NotImplementedError(f'mpRfT2_ESgF can not (target func) '
                                          f'discretize {self._t_.TW.func.__class__}.')

        else:
            raise NotImplementedError(f"mpRfT2_ESgF cannot discretize "
