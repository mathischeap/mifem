...
class LocalNumbering(FrozenOnly):
    ...
    def _2dCSCG_0Form_Inner(self):
        ...
        _ln_ = (np.arange(self._FS_.num_basis._2dCSCG_0Form_Inner[0]).reshape(*p, order='F'),)
        return _ln_
    ...
    def _2dCSCG_0Form_Outer(self):
        p = [self._FS_.p[i]+1 for i in range(self._FS_.ndim)]
        _ln_ = (np.arange(self._FS_.num_basis._2dCSCG_0Form_Outer[0]).reshape(*p, order='F'),)
        return _ln_
    ...
    def ___PRIVATE_matplot_form___(self, what_form, saveto=None, usetex=False):
        ...
        if rAnk == mAster_rank:
            ...
            if 'Inner' in what_form:
                ...
                LN1 = self._2dCSCG_1Form_Inner
                LN2 = self._2dCSCG_2Form_Inner
                orientation = 'Inner'
            elif 'Outer' in what_form:
                LN0 = self._2dCSCG_0Form_Outer
                LN1 = self._2dCSCG_1Form_Outer
                LN2 = self._2dCSCG_2Form_Outer
                orientation = 'Outer'
            else:
                raise Exception()
            k = int(what_form[1])
