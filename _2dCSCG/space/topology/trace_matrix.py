...
class TraceMatrix(FrozenOnly):
    ...
    def _0Trace_Inner(self):
        """ """
        PU = np.zeros((self._FS_.num_basis._0Form_Inner[0], self._FS_.num_basis._0Trace_Inner[0]), dtype=int)
        sln = self.___generate_gathering_hybrid_element___(self._FS_.num_basis._0Trace_Inner[2])
        oln = self._FS_.local_numbering._0Form_Inner[0]
    ...
    def _0Trace_Outer(self):
        """ """
        PU = np.zeros((self._FS_.num_basis._0Form_Outer[0], self._FS_.num_basis._0Trace_Onner[0]), dtype=int)
        sln = self.___generate_gathering_hybrid_element___(self._FS_.num_basis._0Trace_Onner[2])
        oln = self._FS_.local_numbering._0Form_Outer[0]
        PU[oln[0, :].ravel('F'), sln['U']] = +1  # Upper
        PU[oln[-1, :].ravel('F'), sln['D']] = -1  # Down
        PU[oln[:, 0].ravel('F'), sln['L']] = +1  # Left    ##########
        PU[oln[:, -1].ravel('F'), sln['R']] = -1  # Right   #############
    ...
    def _1Trace_Inner(self):
        """ """
        PU = np.zeros((self._FS_.num_basis._1Form_Inner[0], self._FS_.num_basis._1Trace_Inner[0]), dtype=int)
        sln = self.___generate_gathering_hybrid_element___(self._FS_.num_basis._1Trace_Inner[2])
        oln_WE, oln_NS = self._FS_.local_numbering._1Form_Inner
    ...
    def _1Trace_Outer(self):
        """ """
        PU = np.zeros((self._FS_.num_basis._1Form_Outer[0], self._FS_.num_basis._1Trace_Outer[0]), dtype=int)
        sln = self.___generate_gathering_hybrid_element___(self._FS_.num_basis._1Trace_Outer[2])
        oln_NS, oln_WE = self._FS_.local_numbering._1Form_Outer
        PU[oln_NS[0, :].ravel('F'), sln['U']] = +1  # Upper
        PU[oln_NS[-1, :].ravel('F'), sln['D']] = -1  # Down
        PU[oln_WE[:, 0].ravel('F'), sln['L']] = -1  # Left
        PU[oln_WE[:, -1].ravel('F'), sln['R']] = +1  # Right
        T = PU.T
        nbt = self._FS_.num_basis._1Trace_Outer[2]
        T_ = [None, None, None, None]
        m = 0
        for i in range(4):
            T_[i] = T[m:m + nbt['UDLR'[i]], :]
