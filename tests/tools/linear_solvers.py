...
def ___generate_A_b_of_Manguoglu_Paper___():
    ...
    if SIZE > 1:
        assert not A.whether.master_dominating, f"designed to be."
...
def test_LinearSolver_No0_GMRES():
    ...
    x0, info, beta, ITER, message = RegularSolverDistributor("GMRES", routine='sp')(
        A, b, X0, restart=9,
    ...
    x0, info, beta, ITER, message = RegularSolverDistributor("GMRES", routine='auto')(
        A, b, X0, restart=9, preconditioner=None, COD=False,
    )
    x0 = x0.V
    np.testing.assert_array_almost_equal(x0, np.array([-3.23891085,  3.44129703,  1.7765975, -2.7063454, -0.11510028,
                                                       0.94048189,  0.36495389,  0.54018445,  1.57663592]))
    ...
    x0, info, beta, ITER, message = RegularSolverDistributor("GMRES", routine='auto')(
        A, b, X0, restart=9, preconditioner=None, COD=False,
        loading_factor=0
    )  # make sure we use parallel routine
    x0 = x0.V
    np.testing.assert_array_almost_equal(x0, np.array([-3.23891085,  3.44129703,  1.7765975, -2.7063454, -0.11510028,
                                                       0.94048189,  0.36495389,  0.54018445,  1.57663592]))
...
def test_LinearSolver_No1_BiCGSTAB():
    ...
    np.testing.assert_array_almost_equal(
        x0,
        np.array([-2.1810344827586, 1.8362068965517, -0.5948275862068])
    )

    x0, info, beta, ITER, message = \
        RegularSolverDistributor("BiCGSTAB")(
            A, b, X0, maxiter=10, preconditioner=('Jacobi', dict()), COD=False
        )
    x0 = x0.V
    np.testing.assert_array_almost_equal(x0, np.array([-2.1810344827586, 1.8362068965517, -0.5948275862068]))
...
def test_LinearSolver_No2_LooseGMRES():
    ...
    x0, info, beta, ITer, message = RegularSolverDistributor("GCROTmk")(
        A, b, X0, m=20, k=20, atol=1e-9, maxiter=100, preconditioner=None, COD=False
    )
    x0 = x0.V
    np.testing.assert_array_almost_equal(
        x0,
        np.array(
            [-3.23891085,  3.44129703,  1.7765975, -2.7063454, -0.11510028, 0.94048189,
             0.36495389,  0.54018445,  1.57663592]
        )
    )

    x0, info, beta, ITer, message = RegularSolverDistributor("GCROTmk")(
        A, b, X0, m=20, k=20, atol=1e-9,
        preconditioner='spiLU',
    ...
    np.testing.assert_array_almost_equal(
        x0, np.array(
            [-3.23891085,  3.44129703,  1.7765975, -2.7063454, -0.11510028,
             0.94048189,  0.36495389,  0.54018445,  1.57663592]
        )
    )

    x0, info, beta, ITer, message = RegularSolverDistributor("LGMRES")(
        A, b, X0, m=6, k=2, atol=1e-9, maxiter=100, preconditioner=None, COD=False
    )
    x0 = x0.V
    np.testing.assert_array_almost_equal(
        x0, np.array(
    ...
    x0, info, beta, ITER, message = RegularSolverDistributor("LGMRES")(
        A, b, X0, m=6, k=2, atol=1e-9, maxiter=100, preconditioner=('Jacobi', dict()), COD=False
    )
    x0 = x0.V
    np.testing.assert_array_almost_equal(
        x0,
    ...
    x0, info, beta, Iter, message = RegularSolverDistributor("GMRES")(
        A, b, X0, restart=8, atol=1e-9, maxiter=100, preconditioner=None, COD=False
    )
    assert Iter > ITer > ITER

    x0, info, beta, Iter, message = RegularSolverDistributor("LGMRES", routine='sp')(
        A, b, X0, atol=1e-9, maxiter=100, preconditioner=None, COD=False
    )
    x0 = x0.V
    np.testing.assert_array_almost_equal(
        x0, np.array(
            [-3.23891085,  3.44129703,  1.7765975, -2.7063454,
             -0.11510028, 0.94048189,  0.36495389,  0.54018445,  1.57663592]
        )
    )

    x0, info, beta, Iter, message = RegularSolverDistributor("LGMRES", routine='sp')(
        A, b, X0, atol=1e-9, m=10, k=2, maxiter=100, preconditioner='spiLU', COD=False
    )
    x0 = x0.V
    np.testing.assert_array_almost_equal(
        x0, np.array(
    ...
    x0, info, beta, Iter, message = RegularSolverDistributor("TFQMR", routine='sp')(
        A, b, X0, atol=1e-9, maxiter=100, preconditioner='spiLU', COD=False
    )
    x0 = x0.V
    np.testing.assert_array_almost_equal(
        x0, np.array(
    ...
    x0, info, beta, Iter, message = RegularSolverDistributor("TFQMR", routine='sp')(
        A, b, X0, atol=1e-9, maxiter=100, COD=False
    )
    x0 = x0.V
    np.testing.assert_array_almost_equal(
        x0, np.array(
...
def test_LinearSolver_No3_direct():
    ...
    np.testing.assert_array_almost_equal(
        x0,
        np.array([-3.23891085,  3.44129703,  1.7765975, -2.7063454,
                  -0.11510028, 0.94048189,  0.36495389,  0.54018445,  1.57663592]
                 )
    )
